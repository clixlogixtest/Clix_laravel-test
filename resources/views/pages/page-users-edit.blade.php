      ...
      <div class="row">
              ...
              <?php foreach($log as $k=>$logs){
                ...
                if($description == 'A user account is created'){
                 ...
                 <?php
                  ...
                  <?php
                }elseif($description == 'Player wallet balance updated' ){
                  ?>
                   <li class="<?= $clas; ?>">
                  <div class="timeline-badge cyan">
                    <a class="tooltipped" data-position="top" data-tooltip="<?= date('M d Y', strtotime($logs['timestamp'])); ?>"><i
                    ...
                    <div class="card m-0 hoverable">
                      <div class="card-content">
                        ...
                        <div class="divider"></div>
                          ...
                          <?php
                  if($log_before_changes['total_coin'] != $log_details['total_coin']){
                  ?>
                  <p class="card-text mt-1"> Wallet Balance : <?= $log_details['total_coin']; ?>
                  </p>
                  <?php
                  }
                  ?>
        ...
        <div class="col s12" id="view">
          <div>
            ...
            <!-- Phone -->
            <div class="col s12 phone mt-4 p-0">
              <div class="col s2 m2 l3"><i class="material-icons"> place </i> Address</div>
              <div class="col s10 m10 l7">
                <p class="m-0">{{$user['0']['address']}} {{$user['0']['town']}} {{$user['0']['post_code']}}</p>
              </div>
            </div>
            @endif
            ...
            <!-- Phone -->
            <form action="{{route('users.playerWalletBalanceUpdate', $user['0']['id'])}}" method="post">
              {{ csrf_field() }}
            {{ method_field('put') }}
              <div class="col s12 phone mt-4 p-0">
                
                <div class="col s2 m2 l3"><i class="material-icons"> account_balance_wallet </i> Wallet Balance : </div>
                <div class="col s10 m10 l3">
                  <input id="wallet_balance" name="wallet_balance" type="number" class="validate" value="{{$user['0']['total_coin']}}" min="0">
                </div>
                <div class="col s10 m10 l4">
                  <button class="g-recaptcha btn indigo" type="submit">Save</button>
                </div>
              </div>
        ...
        <div class="col s12" id="edit">
          <!-- users add account form start -->
          <form id="userEditForm" action="{{route('users.update', $user['0']['id'])}}" method="post">
            {{ csrf_field() }}
            {{ method_field('put') }}
            <div class="row">
              <div class="col s12">
                <div class="row">
                  ...
                  <div class="col s12 input-field" style="margin-top: 0px;">
                    <h2 for="username" style="color:#9e9e9e; font-size: 1rem; margin: 0rem 0 1.424rem 0;">Roles*</h2>
                    <?php 
                    ...
                    $roleById = $user['0']['role'];
                    $roleById = explode(',', $roleById);
                    if(in_array("competition_administrator", $roleById))
                    {
                      $competition_admin = 'checked';
                    }
                    
                    if (in_array("user_administrator", $roleById))
                    {
                      $user_admin = 'checked';
                    }
                    
                    if (in_array("organisation_administrator", $roleById))
                    {
                      $organisation_admin = 'checked';
                    }
                    
                    if (in_array("prize_administrator", $roleById))
                    {
                      $prize_admin = 'checked';
                    }
                    if (in_array("player", $roleById))
                    {
                      $player = 'checked';
                    }
                    ?>
                  <div style="display: flex;">
                    ...
                    <div style="width: 21%;">
                      <label>
                        <input id="competition_admin" name="role[]" type="checkbox" class="validate" value="competition_administrator" data-error=".errorTxt1" {{$competition_admin}}> <span>Competition Admin</span>
                      </label>
                    </div>
                      
                    <div style="width: 15%;">
                      <label>
                        <input id="user_admin" name="role[]" type="checkbox" class="validate" value="user_administrator" data-error=".errorTxt1" {{$user_admin}}> <span>User Admin</span>
                      </label>
                    </div>
                    
                    <!-- <div style="width: 25%">
                      <label>
                        <input id="organisation_admin" name="role[]" type="checkbox" class="validate" value="organisation_administrator" data-error=".errorTxt1" {{$organisation_admin}}> <span>Organisation Admin</span>
                      </label>
                    </div> -->
                      
                    <div style="width: 21%;">
                      <label>
                        <input id="prize_admin" name="role[]" type="checkbox" class="validate" value="prize_administrator" data-error=".errorTxt1" {{$prize_admin}}> <span>Prize Admin</span>
                      </label>
                    </div>
                    @endif
                    @if($player)
                    <div style="width: 12%;">
                      <label>
                        ...
                        <input id="player" name="role[]" type="checkbox" class="validate" value="player"
                        data-error=".errorTxt1" {{$player}}><span>Player</span>
                      </label>
                    </div>
                    @endif
                    </div>
                    @if ($errors->has('role'))
                      <small class="errorTxt1">{{ $errors->first('role') }}</small>
                    @endif
                  </div>
